...
def test_transport():
    ...
    assert (
        tr.start_to_move() == "t_transport starts to move..."
    ), "Result must be 't_transport starts to move...'"
    assert (
        tr.sound() == "...sound when moving..."
    ), "Result must be '...sound when moving...'"
...
def test_electric_transport():
    ...
    assert (
        el_tr.start_to_move() == "t_el_transport start the engine and go!"
    ), "Result must be 't_el_transport start the engine and go!'"
    assert (
        el_tr.sound() == "...sound when moving..."
    ), "Result must be '...sound when moving...'"
    assert (
        el_tr.stop_transport() == "Transport stopped"
    ), "Result must be 'Transport stopped'"
...
def test_air_transport():
    ...
    assert (
        air_tr.start_to_move() == "air_transport takes off!"
    ), "Result must be 'air_transport takes off!'"
    assert (
        air_tr.sound() == "...sound when moving..."
    ), "Result must be '...sound when moving...'"
...
def test_train():
    ...
    assert (
        train.start_to_move() == "t_train start the engine and go!"
    ), "Result must be 't_train start the engine and go!'"
    assert (
        train.sound() == "Sound when moving chuh-chux tu-tu"
    ), "Result must be 'Sound when moving chuh-chux tu-tu'"
    train.add_wagons(5)
    assert train.count_wagons == 80, "Result must be '80'"
    assert (
        train.stop_transport() == "Transport stopped"
    ), "Result must be 'Transport stopped'"


def test_air_balloon():
    air_balloon = AirBalloon(
        "air_balloon", 1_000_000, 200, flight_altitude=5_000, temperature_inside=70
    )
    ...
    assert (
        air_balloon.start_to_move() == "air_balloon takes off!"
    ), "Result must be 'air_balloon takes off!'"
    assert (
        air_balloon.sound() == "Sound when moving 'phhh-phhh' *sound of air heat burner*"
...
def test_plane():
    ...
    assert plane.length_wing == 73, "Result must be '73'"

    expected_res = "Result must be 'Turbine of Boeing 727 is clean'"
    assert plane.clean_turbine() == "Turbine of Boeing 727 is clean", expected_res
...
def test_plane_start_to_move():
    plane = Plane(
        ...
        750,
        engine="JT8D",
        flight_altitude=14_000,
        length_wing=60,
    )
    expected_res = "Boeing 757 start the engine and takes off!"
    assert (
        plane.start_to_move() == "Boeing 757 start the engine and takes off!"
    ), expected_res
